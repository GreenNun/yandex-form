...
MyForm = {
    validate: function () {
        ...
        if (fio.trim().replace("  ", " ").split(" ").length !== 3) {
            result.isValid = false;
            result.errorFields.push('fio');
        }

        // if (email.match(new RegExp('/^(([^<>()\[\]\\.,;:\s@"]+(\.[^<>()\[\]\\.,;:\s@"]+)*)|(".+"))@(ya.ru|yandex.ru|yandex.ua|yandex.by|yandex.kz|yandex.com'))) {
        //     result.isValid = false;
        //     result.errorFields.push('email');
        // }

        // if (email.match(new RegExp('\+7\(\d{3}\)\d{3}-\d{2}-\d{2}'))) {
        //     result.isValid = false;
